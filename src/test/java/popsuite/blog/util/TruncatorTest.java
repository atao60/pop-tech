...
public class TruncatorTest {
    ...
    private static final String HTML_DOC = String.join(System.lineSeparator(),
...
"</ul><p>Pourquoi pas de fournir un nom, mais aucune envie d&rsquo;une adresse email qui soit rendue publique.</p><p>Ne reste donc qu&rsquo;à remplacer <a href='https://github.com/github/maven-plugins'>Github site plugin</a> par <a href='http://maven.apache.org/plugins/maven-scm-publish-plugin/'>maven-scm-publish-plugin</a>.</p><p>Ah, quel plaisir d&rsquo;avoir enfin son petit blog perso. Et cerise sur le gâteau&nbsp;: <a href='https://github.com/'>Github</a> permet de publier un tel site avec son propre nom de domaine.</p><p>Le dépôt Github du blog est disponible <a href='https://github.com/atao60/pop-tech'>ici</a>.</p><p>Dans une série de billets à venir, je donnerai plus de détail pour arriver à ce résultat. À bientôt.</p>"
    ...
    private static final String SIXTY_WORDS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
                    "<p>Faire un blog sur mes pérégrinations informatiques me trottait dans la tête depuis un certain temps. Histoire d&rsquo;en conserver une trace. Et si en plus cela pouvait servir à d&rsquo;autres&hellip;</p><p>Après avoir installé un <a href='http://atao60.github.io/maven-site-demo/'>site javadoc</a> de démonstration sur <a href='https://github.com/'>Github</a> à l&rsquo;aide de <a href='http://maven.apache.org/'>Maven</a>, la possibilité de créer un blog sur le même principe est...</p>"
                    ));
    private static final String TWENTY_WORDS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
    ...
    private static final String TWENTY_SPACES_AND_CHARS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
                    "<p>Faire un blog sur...</p>"
                    ));
    private static final String TWENTY_CHARS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
                    "<p>Faire un blog sur mes...</p>"
                    ));
    private static final String STRICT_TWENTY_SPACES_AND_CHARS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
                    "<p>Faire un blog sur me...</p>"
                    ));
    private static final String STRICT_TWENTY_CHARS_HTML_DOC = unescapeHtml4(String.join(System.lineSeparator(),
    ...
    private static final String           NEW_ELLIPSIS                               = "[...]";
     ...
     */
    @Test
    public void testWithDefaultWordLimitConstructor() throws Exception {
        int limit = 20;
        String result = new Truncator(20).source(HTML_DOC).run();
        result = removeTagSoupArtifacts(result);
        assertThat(result, equalTo(TWENTY_WORDS_HTML_DOC));
        assertThat(result.split("\\s+|(?<=[j|t|d|l|u|s|n|m])" + Truncator.FRENCH_ELISION).length, equalTo(20));
    }

    @Test
    public void testDefaultEllipsis() throws Exception {
        String result = new Truncator(20).source(HTML_DOC).run();
        result = removeTagSoupArtifacts(result);
    ...
    @Test
    public void testNewEllipsis() throws Exception {
        Truncator truncator = new Truncator(20).source(HTML_DOC);
    ...
    @Test
    public void testWithCharLimitConstructor() throws Exception {
        Truncator truncator = new Truncator(Unit.character, 20, DEFAULT_CHARSET).source(HTML_DOC);
        ...
        assertThat(result.length(), lessThanOrEqualTo(20));
    }
    
    @Test
    public void testWithSpaceAndCharLimitConstructor() throws Exception {
        Truncator truncator = new Truncator(Unit.character, 20, DEFAULT_CHARSET).source(HTML_DOC);
    ...
    @Test
    public void testWithStrictCharLimitConstructor() throws Exception {
        int limit = 20;
        Truncator truncator = new Truncator(Unit.character, limit, DEFAULT_CHARSET).source(HTML_DOC);
    ...
    @Test
    public void testWithStrictSpaceAndCharLimitConstructor() throws Exception {
        int limit = 20;
        Truncator truncator = new Truncator(Unit.character, limit, DEFAULT_CHARSET).source(HTML_DOC);
     ...
     */
    @Test
    public void testIsTruncated() throws Exception {
        Truncator truncator = new Truncator(Unit.character, TruncateContentHandler.NO_LIMIT, DEFAULT_CHARSET).source(HTML_DOC);
        String result = truncator.run();
    ...
    private static String removeTagSoupArtifacts(String o) {
        o = o.replaceAll(" shape=\"rect\"", "");
        o = o.replaceAll("=\"([^\"]*)\"(\\s|>)", "='$1'$2");
        return o.replaceAll("\\n", "");
    }
    ...
    private static String removeEllipsis(String s, String e) {
